...
function renderCards(){
 $("#cardsGrid").innerHTML=state.cards.map(c=>{const l=Number(c.credit_limit),u=cardUsed(c.id),p=l?Math.min(100,u/l*100):0,s=cardStatus(p);return `<article class="credit-card status-${s}" style="--card-neon:${s==="green"?"#b6ff00":s==="yellow"?"#ffe600":s==="orange"?"#ff8a00":"#ff3b5c"}" onclick="showCard('${c.id}')"><span class="card-type">${esc(c.product_type)}</span><h3>${esc(c.name)}</h3><div class="balance">${money(u)}</div><div class="bar"><i style="width:${p}%"></i></div><div class="card-foot"><span>Usado ${p.toFixed(0)}%</span><span>${money(Math.max(0,l-u))} disponible</span></div></article>`}).join("")||'<div class="empty panel">Todavía no tienes tarjetas.</div>';
}
