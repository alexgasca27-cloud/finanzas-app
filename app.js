...
function movementForm(ex=null, forcedType=null){
 ...
 const payload={type:$("#mType").value,concept_name:$("#mConcept").value||null,amount:Number($("#mAmount").value),transaction_date:$("#mDate").value,payment_method:isPay?"debit":pm,is_shared:$("#mShared").value==="true",notes:$("#mNotes").value||null,movement_role:pm==="department_store"?"card_purchase":role,card_id:$("#mCard")?.value||null};
...
function cardForm(){
 showModal(`<form class="form" id="cardForm"><h3>Nueva tarjeta o crédito</h3><label>Nombre<input id="cName" required placeholder="Ej. BBVA Oro"></label><label>Producto<select id="cType"><option>Tarjeta de crédito</option><option>Tarjeta departamental</option><option>Kueski</option></select></label><label>Límite de crédito<input id="cLimit" type="number" min="0" required></label><div id="cardScheduleFields"></div><div class="form-actions"><button type="button" class="danger-btn" onclick="closeModal()">Cancelar</button><button class="primary-btn">Guardar</button></div></form>`);
 const refreshSchedule=()=>{
   const kueski=$("#cType").value==="Kueski";
   $("#cardScheduleFields").innerHTML=kueski
     ? `<label>Día de pago<input id="cDue" type="number" min="1" max="31" placeholder="Ej. 15"></label>`
     : `<div class="schedule-fields"><label>Día de corte<input id="cCut" type="number" min="1" max="31" placeholder="Ej. 10"></label><label>Día de pago<input id="cDue" type="number" min="1" max="31" placeholder="Ej. 30"></label></div>`;
 };
 $("#cType").onchange=refreshSchedule;
 refreshSchedule();
 $("#cardForm").onsubmit=async e=>{e.preventDefault();const type=$("#cType").value;const {error}=await db.from("cards").insert({user_id:state.user.id,workspace_id:state.workspace.id,name:$("#cName").value,product_type:type,credit_limit:Number($("#cLimit").value),cut_day:type==="Kueski"?null:(Number($("#cCut").value)||null),due_day:Number($("#cDue").value)||null});if(error)return notify(error.message);closeModal();loadAll()};
}
...
function showCard(id){
 ...
 let nextPayment=cycle?.nextDue||null, nextPaymentAmount=null, nextPaymentNote="";
 if(type==="Kueski"){
   const upcoming=purchases.map(x=>({x,d:kueskiPaymentDate(x.transaction_date)})).filter(z=>z.d&&z.d>=new Date()).sort((a,b)=>a.d-b.d);
   if(upcoming.length){const d=upcoming[0].d;nextPayment=d.toLocaleDateString("es-MX",{day:"numeric",month:"long",year:"numeric"});nextPaymentAmount=upcoming.filter(z=>z.d.getTime()===d.getTime()).reduce((s,z)=>s+Number(z.x.amount),0);nextPaymentNote="Según la fecha de compra. Las quincenas se configurarán al registrar el financiamiento."}
 ...
 const recent=rows.slice(0,8).map(x=>`<div class="card-detail-row"><div><strong>${esc(x.concept_name||x.description||"Movimiento")}</strong><small>${esc(x.transaction_date)} · ${x.is_shared?"Compartido":"Personal"}</small></div><strong class="${x.movement_role.includes("payment")?"income-amount":"expense-amount"}">${x.movement_role.includes("payment")?"+":"-"}${money(x.amount)}</strong></div>`).join("")||'<div class="empty">Sin movimientos registrados en esta tarjeta.</div>';
 ...
 const schedule=type==="Kueski"?`<div class="detail-section"><div class="detail-section-title">📅 Próximo pago</div><div class="next-payment"><strong>${nextPaymentAmount!==null?money(nextPaymentAmount):"—"}</strong><span>${nextPayment||"Sin fecha calculada"}</span></div><p class="muted tiny">${esc(nextPaymentNote)}</p></div>`:`<div class="detail-section"><div class="detail-section-title">📅 Ciclo de la tarjeta</div><div class="detail-grid"><div><small>Fecha de corte</small><strong>${c.cut_day?`Día ${c.cut_day}`:"No configurada"}</strong></div><div><small>Fecha límite</small><strong>${c.due_day?`Día ${c.due_day}`:"No configurada"}</strong></div><div><small>Periodo actual</small><strong>${cycle?.label||"—"}</strong></div><div><small>Próximo pago</small><strong>${nextPayment||"No configurado"}</strong></div></div></div>`;
 showModal(`<div class="card-detail ${color}"><div class="detail-header"><div><span class="card-type">${esc(type)}</span><h3>${esc(c.name)}</h3></div><div class="detail-usage">${p.toFixed(0)}%<small>uso</small></div></div><div class="detail-summary"><div><small>Límite</small><strong>${money(limit)}</strong></div><div><small>Deuda actual</small><strong>${money(used)}</strong></div><div><small>Disponible</small><strong>${money(available)}</strong></div></div>${schedule}<div class="detail-section"><div class="detail-section-title">🧾 Resumen de compras</div><div class="detail-grid"><div><small>Total compras</small><strong>${money(purchaseTotal)}</strong></div><div><small>Pagos realizados</small><strong>${money(paymentTotal)}</strong></div><div><small>Personal</small><strong>${money(personalTotal)}</strong></div><div><small>Compartido</small><strong>${money(sharedTotal)}</strong></div>${traditional?`<div><small>Compras del periodo</small><strong>${money(periodTotal)}</strong></div>`:""}</div></div>${members}<div class="detail-section"><div class="detail-section-title">🧾 Movimientos recientes</div><div class="card-detail-list">${recent}</div></div></div>`);
}
