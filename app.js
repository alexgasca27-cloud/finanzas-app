...
function showGoal(id){const g=state.goals.find(x=>x.id===id);if(!g)return;const d=goalData(g);const freqLabel=d.freq==="biweekly"?"quincena":"mes";const rows=d.contribs.map(x=>`<div class="goal-contribution"><div><strong>${money(x.amount)}</strong><small>${esc(x.contribution_date)}${x.note?` · ${esc(x.note)}`:""}</small></div><div class="actions"><button type="button" class="icon-btn" title="Editar aportación" onclick="event.stopPropagation();editContribution('${x.id}')">✎</button><button type="button" class="icon-btn" title="Eliminar aportación" onclick="event.stopPropagation();deleteContribution('${x.id}','${g.id}')">×</button></div></div>`).join("")||'<div class="empty">Todavía no hay aportaciones registradas.</div>';showModal(`<div class="goal-detail"><div class="detail-header"><div><span class="card-type">META DE AHORRO</span><h3>${esc(g.name)}</h3></div><div class="detail-usage">${Math.min(100,d.target?d.saved/d.target*100:0).toFixed(0)}%<small>avance</small></div></div><div class="detail-summary"><div><small>Meta</small><strong>${money(d.target)}</strong></div><div><small>Ahorrado</small><strong>${money(d.saved)}</strong></div><div><small>Falta</small><strong>${money(d.remaining)}</strong></div></div><div class="goal-progress-big"><div class="bar"><i style="width:${Math.min(100,d.target?d.saved/d.target*100:0)}%"></i></div></div><div class="goal-estimate-large">${goalEstimateText(d)}${d.planned?`<small>Con ${money(d.planned)} por ${freqLabel}, recalculada automáticamente con cada aportación.</small>`:""}</div><div class="form-actions"><button class="primary-btn" onclick="closeModal();contributionForm('${g.id}')">+ Registrar aportación</button><button class="danger-btn" onclick="deleteGoal('${g.id}')">🗑 Eliminar meta</button><button class="secondary-btn" onclick="closeModal()">Cerrar</button></div><div class="detail-section"><div class="detail-section-title">💰 Aportaciones</div><div class="goal-contributions">${rows}</div></div></div>`)}
async function editContribution(id){
 const x=state.goalContributions.find(v=>v.id===id); if(!x)return;
 showModal(`<form class="form" id="editContributionForm"><h3>Editar aportación</h3><label>Monto<input id="egcAmount" type="number" min="0.01" step="0.01" required value="${Number(x.amount||0)}"></label><label>Fecha<input id="egcDate" type="date" required value="${esc(x.contribution_date||todayISO())}"></label><label>Nota (opcional)<input id="egcNote" value="${esc(x.note||"")}" placeholder="Ej. Aportación extraordinaria"></label><div class="form-actions"><button type="button" class="danger-btn" onclick="closeModal()">Cancelar</button><button class="primary-btn">Guardar cambios</button></div></form>`);
 $("#editContributionForm").onsubmit=async e=>{e.preventDefault();const amount=Number($("#egcAmount").value);if(!amount||amount<=0)return notify("El monto debe ser mayor a 0.");const {error}=await db.from("savings_goal_contributions").update({amount,contribution_date:$("#egcDate").value,note:$("#egcNote").value.trim()||null}).eq("id",id).eq("workspace_id",state.workspace.id);if(error)return notify(error.message);const goalId=x.goal_id;closeModal();await loadAll();showGoal(goalId)}
}
async function deleteContribution(id,goalId){if(!confirm("¿Eliminar esta aportación? La fecha estimada de la meta se recalculará automáticamente."))return;const {error}=await db.from("savings_goal_contributions").delete().eq("id",id).eq("workspace_id",state.workspace.id);if(error)return notify(error.message);await loadAll();showGoal(goalId)}
async function deleteGoal(id){const g=state.goals.find(x=>x.id===id);if(!g)return;if(!confirm(`¿Eliminar la meta “${g.name}”? También se eliminarán sus aportaciones registradas. Esta acción no se puede deshacer.`))return;const {error}=await db.from("savings_goals").delete().eq("id",id).eq("workspace_id",state.workspace.id);if(error)return notify(error.message);closeModal();await loadAll()}
