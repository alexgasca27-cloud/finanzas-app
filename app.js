...
function showCard(id){
 ...
 const statementSection=traditional&&statement?.info?`<div class="detail-section statement-section"><div class="detail-section-title">📄 Estado de cuenta</div><div class="statement-head"><div><small>Periodo</small><strong>${statement.info.periodLabel}</strong></div><div><small>Fecha de corte</small><strong>${statement.info.cutDate.toLocaleDateString("es-MX",{day:"numeric",month:"long",year:"numeric"})}</strong></div><div><small>Fecha límite</small><strong>${statement.info.dueDate.toLocaleDateString("es-MX",{day:"numeric",month:"long",year:"numeric"})}</strong></div></div><div class="statement-amount"><div><small>Pago del estado de cuenta</small><strong>${money(statement.pending)}</strong></div><button type="button" class="primary-btn" onclick="markCardPaid('${c.id}')">✓ Tarjeta pagada</button></div><div class="statement-breakdown"><span>Compras del estado de cuenta <strong>${money(statement.gross)}</strong></span><span>Pagos registrados <strong>${money(statement.paid)}</strong></span><span>Pendiente <strong>${money(statement.pending)}</strong></span></div><p class="muted tiny">Al marcarla como pagada se registra el pago y el siguiente periodo conserva las compras nuevas; la tarjeta no vuelve a cero.</p></div>`:"";
...
async function markCardPaid(id){
 ...
 if(st.pending<=0)return notify("Esta tarjeta no tiene un pago pendiente del último estado de cuenta.");
 const label=st.info.dueDate.toLocaleDateString("es-MX",{day:"numeric",month:"long",year:"numeric"});
 if(!confirm(`¿Marcar ${c.name} como pagada?\n\nPago del estado de cuenta: ${money(st.pending)}\nFecha límite: ${label}\n\nSe registrará un pago y el nuevo periodo conservará las compras que ya hayas realizado.`))return;
 const {error}=await db.from("movements").insert({user_id:state.user.id,workspace_id:state.workspace.id,type:"expense",concept_name:`Pago ${c.name}`,amount:Number(st.pending.toFixed(2)),transaction_date:todayISO(),payment_method:"debit",is_shared:false,notes:"Pago de estado de cuenta",movement_role:"card_payment",card_id:id});
 if(error)return notify(error.message);
 closeModal();await loadAll();
}
