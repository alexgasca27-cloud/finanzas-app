...
function cardMsiSchedule(purchase,card){const n=Math.max(1,Number(purchase.card_installments)||1),amount=Number(purchase.amount)||0,first=cardDueDateForPurchase(card,purchase.transaction_date);if(n<=1||!first)return [];return Array.from({length:n},(_,i)=>{const d=new Date(first.getFullYear(),first.getMonth()+i,clampDay(first.getFullYear(),first.getMonth()+i+1,first.getDate()));return {date:d,amount:amount/n,index:i+1,total:n}});}
