...
googleButton.addEventListener('click',async()=>{
  ...
  if(error){
    console.error(error);
    status.textContent='No se pudo iniciar sesión con Google.';
    googleButton.disabled=false;
  }
});

appleButton.addEventListener('click',()=>{
  status.textContent='El inicio de sesión con Apple lo configuraremos después.';
});
...
(async()=>{
  ...
  if(error){console.error(error);showLogin('No pudimos comprobar tu sesión.');return;}
