...
function showCard(id){
 ...
 if(type==="Kueski"){
   const upcoming=purchases.flatMap(x=>kueskiSchedule(x).map(item=>({x,...item}))).filter(z=>z.date&&z.date>=new Date()).sort((a,b)=>a.date-b.date);
   if(upcoming.length){const d=upcoming[0].date;const sameDay=upcoming.filter(z=>z.date.getTime()===d.getTime());nextPayment=d.toLocaleDateString("es-MX",{day:"numeric",month:"long",year:"numeric"});nextPaymentAmount=sameDay.reduce((s,z)=>s+z.amount,0);nextPaymentNote=`Incluye las quincenas programadas de las compras pendientes. Cada compra se divide entre 1 y 12 quincenas.`}
